...
class InputDetailsComponent extends Component<InputDetailsProps, InputDetailsState> {
  ...
  componentDidMount() {
    ...
    this.websocket
      .connect(`input/${this.props.id}`, (data: MessageBase) => {
            ...
            <Grid item xs={6} md={4} lg={3} key={i}>
              <Paper>
                <Typography align="center" variant="body1">
                  {Translation.translate('DPad') + ' ' + (i + 1)}
                </Typography>
  ...
  getButtons(inputDetails: InputDeviceInputDetails) {
    ...
    return (
      <>
        <Typography className={classes.header} variant="h5">
          {Translation.translate('Buttons')}
        </Typography>
        <Grid container spacing={3}>
          {inputDetails.sources
            .filter((s) => s.type === 'button')
            .map((s) => (
              <Grid item xs={6} md={4} lg={3} key={s.offset}>
                <Paper>
                  <Typography align="center" variant="body1">
                    {s.name}
                  </Typography>
                  <LinearProgress
                    variant="determinate"
                    value={this.getValue(inputDetails.inputMethod, s.offset)}
  ...
  getAxes(inputDetails: InputDeviceInputDetails) {
    ...
    return (
      <>
        <Typography className={classes.header} variant="h5">
          {Translation.translate('Axes')}
        </Typography>
        <Grid container spacing={3}>
          {inputDetails.sources
            .filter((s) => s.type === 'axis')
            .map((s) => (
              <Grid item xs={6} md={4} lg={3} key={s.offset}>
                <Paper>
                  <Typography align="center" variant="body1">
                  ...
                  <LinearProgress
                    variant="determinate"
                    value={this.getValue(inputDetails.inputMethod, s.offset, 0.5)}
                    classes={{ bar: classes.bar }}
                  />
                </Paper>
              </Grid>
  ...
  getForceFeedbacks(inputDetails: InputDeviceInputDetails) {
    ...
    return (
      <>
        <Typography className={classes.header} variant="h5">
          {Translation.translate('ForceFeedbacks')}
        </Typography>
        <Grid container spacing={3}>
          {inputDetails.forceFeedbacks.map((s) => (
            <Grid item xs={6} md={4} lg={3} key={s.offset}>
              <Paper>
                <Typography align="center" variant="body1">
                  {s.offset}
    ...
    if (!this.state.device) {
      ...
      const summary = (
        <Paper className={classes.paper}>
          <p>
            {Translation.translate('input.name')}: {this.state.device.name}
          </p>
          <p>
            {Translation.translate('input.id')}: {this.state.device.id}
          </p>
          <p>
            {Translation.translate('input.hardwareid')}: {this.state.device.hardwareId}
          </p>
      ...
      content = (
        <>
          {summary}
          {this.state.device.inputs.map((i) => (
            <div className={classes.paper} key={i.inputMethod}>
              <Typography className={classes.header} variant="h4">
                {this.mapMethod(i.inputMethod)}
              </Typography>
              <Switch
                color="primary"
                checked={i.running}
